    ...
    if request.method == 'POST':
        ...
        if newTransition is None:
            ...
            if(newTransition.isParametersEmpty()):
                ...
                request.app['regrSock'].send_json(getStretegyCustomizeParametersReq)
                respMsg = request.app['regrSock'].recv_json()
                msgName = respMsg[0]
                payload = respMsg[1]
                parameters = ''
...
async def getStrategies(request):
    ...
    request.app['regrSock'].send_json(getStrategyListReq)
    getStrategyListCfm =  request.app['regrSock'].recv_json()
...
async def getStrategyData(request):
    ...
    if data == 'sourceCode':
        ...
        request.app['regrSock'].send_json(getStrategySourceCodeReq)
        respMsg = request.app['regrSock'].recv_json()
        msgName = respMsg[0]
        payload = respMsg[1]
